/**
 * TreeView - Alpine.js component for hierarchical tree view with drag and drop
 * @version 1.0.11 - Fixed nodeName.toLowerCase error
 */
document.addEventListener('alpine:init', () => {
    Alpine.data('TreeView', (config = {}) => {
        ...
        return {
            // Core data
            treeData: Array.isArray(options.data) ? options.data : [],
            jsonOutput: '',
            ...
            init() {
                ...
                // If no data was provided, load example data
                            ...
                            children: [
                                {
                                    ...
                                    expanded: true,
                                    children: [
                                        { id: 'node-3', name: 'Facebook', expanded: true, children: [] },
                                        { id: 'node-4', name: 'Twitter', expanded: true, children: [] }
                                    ]
                                },
                                { id: 'node-5', name: 'Email Campaigns', expanded: true, children: [] }
                            ]
                        },
                        {
                            id: 'node-6',
                            ...
                            expanded: true,
                            children: [
                                { id: 'node-7', name: 'B2B', expanded: true, children: [] },
                                { id: 'node-8', name: 'B2C', expanded: true, children: [] }
                            ]
                        }
                    ];
                }
                ...
                // Set up event listeners for node operations
                this.setupEventListeners();
            },
            
            /**
             * Setup event listeners for consistent updates
             */
            setupEventListeners() {
                // Listen for node operation events
                document.addEventListener('treeview:node-moved', () => {
                    setTimeout(() => {
                        this.updateNodeVisibility();
                        this.redrawTree();
                    }, 10);
                ...
                document.addEventListener('treeview:node-added', () => {
                    setTimeout(() => {
                        this.updateNodeVisibility();
                        this.redrawTree();
                    }, 10);
                ...
                document.addEventListener('treeview:node-deleted', () => {
                    setTimeout(() => {
                        this.updateNodeVisibility();
                        this.redrawTree();
                    }, 10);
            ...
            updateNodeVisibility() {
                ...
                const setVisibilityAttributes = (nodes, currentDepth = 1) => {
                    nodes.forEach(node => {
                        ...
                        node._depth = currentDepth; // Store depth for easier reference
            ...
            /**
             * Watch for X-show visibility changes - FIXED to prevent TypeError with nodeName
             */
                ...
                setTimeout(() => {
                    ...
                    const observer = new MutationObserver((mutations) => {
                        ...
                        for (const mutation of mutations) {
                            // Check if mutation target is a valid DOM element with a real nodeName
                            if (!mutation.target || typeof mutation.target !== 'object') continue;
                            
                            // Safe check for nodeName property
                            ...
                            if (mutation.type === 'attributes' && 
                               (mutation.attributeName === 'style' || 
                                ...
                                // Safe extraction of node ID
                                const nodeId = mutation.target.id ? mutation.target.id.replace('node-', '') : null;
                                if (!nodeId) continue;
                    ...
                    treeContainers.forEach(container => {
                        observer.observe(container, { 
                            ...
                            subtree: true // Need to observe all child elements too
                        });
                    });
                }, 500); // Reduced delay for initial setup
            },
            ...
            /**
             * Get flattened nodes for select dropdowns - NOW WITH DEPTH INFORMATION
             */
            get flattenedNodes() {
                const result = [];
                ...
                const flatten = (nodes, level = 0) => {
                    nodes.forEach(node => {
                        const nodeDepth = node._depth || this.getNodeDepth(node[options.idField]);
                        result.push({ 
                            id: node[options.idField], 
                            name: node[options.nameField], 
                            level,
                            depth: nodeDepth,
                            exceeds: nodeDepth > this.getMaxVisibleDepth()
                        });
            ...
            /**
             * Format node text with search highlighting - now safer
             */
            formatNodeText(node) {
                ...
                const nodeName = node[options.nameField];
                if (typeof nodeName !== 'string') return String(nodeName || '');
                
                const text = nodeName;
                
                // If search highlighting is disabled or there's no search query, return plain text
                if (!options.highlightSearch || !this.searchQuery || !this.searchQuery.trim()) {
                    return text;
                }
                ...
                try {
                    ...
                    console.error('Error in formatNodeText:', error);
